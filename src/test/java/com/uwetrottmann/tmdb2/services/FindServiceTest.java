...
public class FindServiceTest extends BaseTestCase {
    ...
    public void test_find_movie() throws IOException {
        Observable<FindResults> observable = getManager().findService().find(MOVIE2_IMDB_ID, ExternalSource.IMDB_ID, null);

        observable.subscribe(new Action1<FindResults>() {
            @Override
    ...
    public void test_find_people() throws IOException {
        Observable<FindResults> observable = getManager().findService().find(TestData.PERSON_IMDB_ID, ExternalSource.IMDB_ID, null);
    ...
    public void test_find_tv_show() throws IOException {
        Observable<FindResults> observable = getManager().findService().find(TestData.TVSHOW_IMDB_ID, ExternalSource.IMDB_ID, null);
    ...
    public void test_find_tv_season() throws IOException {
        Observable<FindResults> observable = getManager().findService().find(String.valueOf(TestData.TVSHOW_SEASON1_ID),
    ...
    public void test_find_tv_episode() throws IOException {
        Observable<FindResults> observable = getManager().findService().find(TestData.EPISODE_IDMB_ID, ExternalSource.IMDB_ID, null);
